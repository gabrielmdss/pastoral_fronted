...
export class AuthApiService implements AuthApiPort {
  ...
  login(input: LoginInput): Observable<LoginResult> {
    return this.http.post<LoginResponseDto>(`${this.config.apiBaseUrl}/auth/login`, input).pipe(map(({ data }) => ({ accessToken: data.accessToken, usuario: mapAuthenticatedUser(data.usuario) })));
  }
  me(): Observable<AuthenticatedUser> {
    return this.http.get<MeResponseDto>(`${this.config.apiBaseUrl}/auth/me`).pipe(map(({ data }) => mapAuthenticatedUser(data)));
  }
}
