...
describe('Acesso a Planejamento', () => {
  ...
  it.each([false, true])('menu e rotas respeitam ESTOQUE_VISUALIZAR: %s', (allowed) => {
    ...
    expect(!!fixture.nativeElement.querySelector('a[href="/planejamentos"]')).toBe(allowed);
    const routes = appRoutes.find((r) => r.children)?.children ?? [];
    for (const path of ['planejamentos', 'planejamentos/novo', 'planejamentos/:id']) {
